...
namespace THNETII.Diagnostics.DiagnosticSourceExtensions
{
    ...
    public static class DiagnosticSourceExtensions
    {
        ...
        /// If <paramref name="diagnosticSource"/> is <c>null</c>, or if calling <see cref="DiagnosticSource.IsEnabled(string)"/> on <paramref name="diagnosticSource"/>
        /// with <paramref name="name"/> returns <c>false</c>.
        /// </returns>
        /// <remarks>
        ...
        /// <para>If <paramref name="diagnosticSource"/> is <c>null</c>, <c>null</c> is returned.</para>
        /// </remarks>
        ...
        /// <param name="arg2">Optional. An object that represents the additional context for <see cref="DiagnosticSource.IsEnabled(string, object, object)"/>. <c>null</c> by default.</param>
        /// <returns>
        /// A <see cref="DiagnosticSourceWriter"/> using <paramref name="diagnosticSource"/> and <paramref name="name"/>.
        /// If <paramref name="diagnosticSource"/> is <c>null</c>, or if calling <see cref="DiagnosticSource.IsEnabled(string, object, object)"/> on <paramref name="diagnosticSource"/>
        /// with <paramref name="name"/> returns <c>false</c>.
        /// </returns>
        /// <remarks>
        /// This extension method serves as a convenience method to condense the otherwise recommended <c>if</c>-block for logging to a <see cref="DiagnosticSource"/> instance.
